...
%><%--
  @Summary: ArticleMS display page
  @Category: Generated
...
%><% ArticleMS obj = (ArticleMS)request.getAttribute(PortalManager.PORTAL_PUBLICATION); %><%
%><%@ include file='/front/doFullDisplay.jspf' %>
<div class="fullDisplay ArticleMS <%= obj.canBeEditedFieldByField(loggedMember) ? "unitFieldEdition" : "" %>">
<%@ include file='/front/publication/doPublicationHeader.jspf' %>
<table class="fields">
  <tr class="field summary textareaEditor abstract <%= Util.isEmpty(obj.getSummary(userLang)) ? "empty" : "" %>">
    <td class='field-label'><%= channel.getTypeFieldLabel(ArticleMS.class, "summary", userLang) %><jalios:edit pub='<%= obj %>' fields='summary'/></td>
  ...
  <tr class="field intro textareaEditor  <%= Util.isEmpty(obj.getIntro(userLang)) ? "empty" : "" %>">
    <td class='field-label'><%= channel.getTypeFieldLabel(ArticleMS.class, "intro", userLang) %><jalios:edit pub='<%= obj %>' fields='intro'/></td>
  ...
  <tr class="field picture imageEditor  <%= Util.isEmpty(obj.getPicture(userLang)) ? "empty" : "" %>">
    <td class='field-label'><%= channel.getTypeFieldLabel(ArticleMS.class, "picture", userLang) %><jalios:edit pub='<%= obj %>' fields='picture'/></td>
  ...
  <tr class="field content wysiwygEditor  <%= Util.isEmpty(obj.getContent(userLang)) ? "empty" : "" %>">
    <td class='field-label'><%= channel.getTypeFieldLabel(ArticleMS.class, "content", userLang) %><jalios:edit pub='<%= obj %>' fields='content'/></td>
  ...
  <tr class="field relatedContents linkEditor  <%= Util.isEmpty(obj.getRelatedContents()) ? "empty" : "" %>">
    <td class='field-label'><%= channel.getTypeFieldLabel(ArticleMS.class, "relatedContents", userLang) %><jalios:edit pub='<%= obj %>' fields='relatedContents'/></td>
    <td class='field-data' >
  ...
  <tr class="field tag linkEditor  <%= Util.isEmpty(obj.getTag()) ? "empty" : "" %>">
    <td class='field-label'><%= channel.getTypeFieldLabel(ArticleMS.class, "tag", userLang) %><jalios:edit pub='<%= obj %>' fields='tag'/></td>
    <td class='field-data' >
            <% if (Util.notEmpty(obj.getTag())) { %>
            <ol>
              <jalios:foreach name="itData" type="generated.Tag" array="<%= obj.getTag() %>">
              <jalios:if predicate='<%= itData != null && itData.canBeReadBy(loggedMember) %>'>
              <li>
              <jalios:link data='<%= itData %>'/>
              </li>
