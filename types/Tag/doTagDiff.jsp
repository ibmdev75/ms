...
<%--
  @Summary: generated.Tag diff page
  @Category: Generated
...
<% 
{ 
generated.Tag newObj = (generated.Tag)request.getAttribute("newPub"); 
generated.Tag oldObj = (generated.Tag)request.getAttribute("oldPub"); 
String[] vids = (String[])request.getAttribute("vids"); 
int fieldCount = 1; 
TypeEntry typeEntry = channel.getTypeEntry(generated.Tag.class);
...
<% } %>
  ...
  <tr class="vTop">
    ...
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.Tag.class, "description", userLang) %>
      <jalios:if predicate='<%= channel.isMultilingual() %>'><jalios:lang lang='<%= itLang %>'/></jalios:if>
      <% fieldDomId += "_" + itLang; %>
    </td>
    <% 
  ...
  <%-- ** couleurFond ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getCouleurFond(), oldObj.getCouleurFond()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.Tag.class, "couleurFond", userLang) %>
    </td>
    <td><%= Util.getString(newObj.getCouleurFond(), "") %></td>
    <td><%= Util.getString(oldObj.getCouleurFond(), "") %></td>
  </tr>
  </jalios:if>
  <%-- ** couleurTexte ***************** --%>
  <jalios:if predicate='<%= !Util.isSameContent(newObj.getCouleurTexte(), oldObj.getCouleurTexte()) %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap">
      <%= channel.getTypeFieldLabel(generated.Tag.class, "couleurTexte", userLang) %>
    </td>
    <td><%= Util.getString(newObj.getCouleurTexte(), "") %></td>
    <td><%= Util.getString(oldObj.getCouleurTexte(), "") %></td>
  </tr>
  </jalios:if>
  <%-- ** compteur ***************** --%>
  <jalios:if predicate='<%= newObj.getCompteur() != oldObj.getCompteur() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.Tag.class, "compteur", userLang) %></td>
    <td><%= newObj.getCompteur()  %></td>
    <td><%= oldObj.getCompteur()  %></td>
  </tr>
  </jalios:if>
  <%-- ** url ***************** --%>
  <jalios:if predicate='<%= newObj.getUrl() != oldObj.getUrl() %>'>
  <tr class="vTop">
    <td class="text-right"><%= fieldCount++ %>.</td>
    <td nowrap="nowrap"><%= channel.getTypeFieldLabel(generated.Tag.class, "url", userLang) %></td>
    <td>
      <% Data newLink = newObj.getUrl(); %>
      <% if (newLink != null) { %> 
