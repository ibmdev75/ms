...
<div class="fullDisplay PortletWYSIWYG <%= obj.canBeEditedFieldByField(loggedMember) ? "unitFieldEdition" : "" %>">
...
<table class="fields">
  <tr class="field wysiwyg wysiwygEditor  <%= Util.isEmpty(obj.getWysiwyg(userLang)) ? "empty" : "" %>">
    ...
    <td class='field-data' <%= gfla(obj, "wysiwyg") %>>
            <jalios:if predicate='<%= Util.notEmpty(obj.getWysiwyg(userLang)) %>'>
            <jalios:wysiwyg><%= obj.getWysiwyg(userLang) %></jalios:wysiwyg>            
            </jalios:if>
    </td>
  </tr>
