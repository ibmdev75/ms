...
(function(proto) {
    ...
    function toISOString() {
        return this.getUTCFullYear() + '-' + (this.getUTCMonth() + 1)
            .toPaddedString(2) + '-' + this.getUTCDate()
            .toPaddedString(2) + 'T' + this.getUTCHours()
            .toPaddedString(2) + ':' + this.getUTCMinutes()
            .toPaddedString(2) + ':' + this.getUTCSeconds()
            .toPaddedString(2) + 'Z';
    }
...
Object.extend(String.prototype, (function() {
    var NATIVE_JSON_PARSE_SUPPORT = window.JSON && typeof JSON.parse === 'function' && JSON.parse('{"test": true}')
        .test;
    ...
    function gsub(pattern, replacement) {
        ...
        if (!(pattern.length || pattern.source)) {
            ...
            return replacement + source.split('')
    ...
    function toQueryParams(separator) {
        var match = this.strip()
            .match(/([^?#]*)(#.*)?$/);
        if (!match) return {};

        return match[1].split(separator || '&')
    ...
    function times(count) {
        return count < 1 ? '' : new Array(count + 1)
            .join(this);
    }
...
var Enumerable = (function() {
    ...
    function inspect() {
        return '#<Enumerable:' + this.toArray()
            .inspect() + '>';
    }
...
(function() {
    ...
    function reverse(inline) {
        return (inline === false ? this.toArray() : this)
            ._reverse();
    }
    ...
    function inspect() {
        return '[' + this.map(Object.inspect)
            .join(', ') + ']';
    }
...
var Hash = Class.create(Enumerable, (function() {
    ...
    function inspect() {
        return '#<Hash:{' + this.map(function(pair) {
            return pair.map(Object.inspect)
                .join(': ');
        })
            .join(', ') + '}>';
    }
...
Ajax.Request = Class.create(Ajax.Base, {
    ...
    request: function(url) {
        ...
        try {
            ...
            if (this.options.asynchronous) this.respondToReadyState.bind(this)
                .defer(1);
...
Ajax.Response = Class.create({
    ...
    _getResponseJSON: function() {
        ...
        if (!options.evalJSON || (options.evalJSON != 'force' && !(this.getHeader('Content-type') || '')
            .include('application/json')) || this.responseText.blank()) return null;
        try {
...
Ajax.PeriodicalUpdater = Class.create(Ajax.Base, {
    ...
    updateComplete: function(response) {
        ...
        this.timer = this.onTimerEvent.bind(this)
...
(function(GLOBAL) {
    ...
    function update(element, content) {
        ...
        if (Object.isElement(content)) return element.update()
            .insert(content);
    ...
    function insertContentAt(element, content, position) {
        ...
        content = Object.toHTML(content);
        var tagName = ((position === 'before' || position === 'after') ? element.parentNode : element)
            .tagName.toUpperCase();
  ...
  // FIX:JALIOS https://prototype.lighthouseapp.com/projects/8886/tickets/993-bug-in-readattribute-for-form-with-input-named-children#ticket-993-1
  // No change from release (this comment is juste to remember previous fix)
    ...
    function hasClassName(element, className) {
        ...
        return getRegExpForClassName(className)
    ...
    function _getFlag(element, attribute) {
        return $(element)
            .hasAttribute(attribute) ? attribute : null;
    }
...
(function() {
    ...
    Element.Layout = Class.create(Hash, {
        ...
        toObject: function() {
            var args = $A(arguments);
            var keys = (args.length === 0) ? Element.Layout.PROPERTIES : args.join(' ')
                .split(' ');
            var obj = {};
    ...
    if ('getBoundingClientRect' in document.documentElement) {
        Object.extend(Element.Layout.COMPUTATIONS, {
            ...
            'bottom': function(element) {
                ...
                return (pRect.bottom - rect.bottom)
                    .round();
            }
        });
    }
...
(function(engine) {
    ...
    function match(element, selector) {
        return engine.matches(selector, [element])
            .length == 1;
    }
...
(function(GLOBAL) {
    ...
    var methods = Object.keys(Event.Methods)
...
(function() {
    ...
    Object.extend(Selector, {
        ...
        findChildElements: function(element, expressions) {
            var selector = expressions.toArray()
                .join(', ');
