...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		render : function(nst) {
			...
			if (s.add_unload_trigger) {
				t._beforeUnload = tinyMCE.onBeforeUnload.add(function() {
			      ...
			      // Jalios modification to support unload_trigger_option for wikiwyg
					  // see detail explanation in comment of issue JCMS-2981
			      if (!t.isWikiwyg) {
		...
		initContentBody : function() {
			...
			// JALIOS new : use a custom startContentDirty variable initialized from getContent, but in which events are not invoked, to be homogenous with implementation of method isDirty() 
			self.startContentDirtyCheck = self.getContent({format : 'raw', no_events : 1})
		...
		getContent : function(args) {
			...
			if (args.format != 'text') {
        // Jalios modification to prevent trim when working with wikiwyg
        if (!this.isWikiwyg) {
          args.content = tinymce.trim(content);
        } else {
          args.content = content;
		...
		isDirty : function() {
			...
			// JALIOS : do not use 'self.startContent', but instead custom 'self.startContentDirtyCheck' (search all occurence in this file)
			// indeed, default implementation of TinyMCE invoke getContent in two different ways (no_events) 
			// which always leads to different value of startContent & current content when the jcms plugin is used, even when value was not modified at all  
			return tinymce.trim(self.startContentDirtyCheck) != tinymce.trim(self.getContent({format : 'raw', no_events : 1})) && !self.isNotDirty;
		},
