...
(function() {
	...
	var transferState = function(oldEditor, newEditor, bookmark) {
		...
		// Jalios custom : when setContent/getContent is invoked by the original
		// fullscreen plugin, 'raw' format is always specified, but it does not
		// work properly with wikiwyg editor, so apply custom behavior
		var contentFormat = oldEditor.getParam('jcms_wikiwyg') ? undefined : 'raw';
		
		newEditor.setContent(oldEditor.getContent({format : contentFormat}), {format : contentFormat});
