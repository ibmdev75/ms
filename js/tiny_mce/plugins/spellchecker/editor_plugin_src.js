...
(function() {
	...
	tinymce.create('tinymce.plugins.SpellcheckerPlugin', {
      ...
      // Start Jalios custom code
      // Update selected language using 'lang' attribute of textarea/div being worked on
      // The selected language is also stored in current settings for compatibility with fullscreen editor
      // When the fullscreen editor is created it is a new editor with settings copied from the original
      var eltLang = ed.getParam('jcms_spellchecker_language') || (ed.getElement ? ed.getElement().getAttribute('lang') : null);
      ed.settings.jcms_spellchecker_language = eltLang;
      if (eltLang && ed.getParam('spellchecker_languages', '').indexOf('='+eltLang+'') != -1) {
        t.selectedLang = eltLang;
      } 
      // End Jalios custom code
		},
      ...
      // Jalios custom start
      var rpc_class = t.editor.getParam("spellchecker_rpc_class", "");
      if (rpc_class !== '') {
        m = rpc_class + '.' + m;
      }
      // Jalios custom end
