  ...
  Object.extend(WikiToolbar,{
    ...
    runInsertImage: function(event){
      ...
      this.currentSelection = InputUtil.getSelection(context.input,true);
      var wsid = Element.getJcmsId(document.body,'WS_');
      jQuery.jalios.ui.Widget.Chooser.openDataChooser('media', WikiToolbar.insertImageUrlCallback, {
        params : { 'ws': wsid, 'media': 'image' }
      });
      return false;
    },
    ...
    showPreview: function(event) {
      ...
      var url    = JcmsJsContext.getContextPath()+'/jcore/wiki/wiki2html.jsp';
      var params = 'wiki='+encodeURIComponent(input.value);
      Ajax.performAjaxRequest(url,params, function(returnValue){
        preview.innerHTML = returnValue.responseText;
        
        var adjust = function(){ preview.scrollTop = (input.scrollTop * preview.scrollHeight) / input.scrollHeight; };
        adjust.defer();
